...
class Player
  ...
  attr_reader :score # bonus1

  def initialize
    set_name
    # @score = 0 # bonus1
    @score = Score.new # bonus1x
  end
...
# begin bonus1
  def count_score
    if human.move > computer.move
      # human.score += 1
      human.score.plus_equals(5) # bonus1x
    elsif computer.move > human.move
      # computer.score += 1
      computer.score.plus_equals(1) # bonus1x
    end
  end

  def display_score # could move this method to the Player class
    # puts "Your score is #{human.score}; "\
    #   "the computer's score is #{computer.score}"
    puts "Your score is #{human.score.to_s}; "\
      "the computer's score is #{computer.score.to_s}" # bonus1x
  end
...
# end bonus1
  ...
  def play
    ...
    loop do
      ...
      # reset_scores # bonus1
      human.score.reset_score # bonus1x
      computer.score.reset_score # bonus1x
    end
